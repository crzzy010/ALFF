...
# httplib.HTTPConnection._http_vsn_str = 'HTTP/1.0'r
...
if '__main__' == __name__:
	...
	# class depth=1
	sub = ET.SubElement(root, 'Classes', {"sunid":"310555","href":"http://scop.berkeley.edu/sunid=310555", "name":"Artifacts"})

	#startTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')

	startTime = time.time()

	travel_get("http://scop.berkeley.edu/sunid=310555", 1, sub)
	
	# travel_get("http://scop.berkeley.edu/sunid=88552", 6, sub)

	tree = ET.ElementTree(root)
	tree.write('l.xml', encoding="us-ascii", xml_declaration=True, default_namespace=None, method="xml", short_empty_elements=False)
	
	with open("results_l.txt", 'w') as f:
		strs = "acc_num=" + str(acc_num) + "\n" + "pdb num = " + str(count)
		f.write(strs)
	with open("link_l.txt", 'w') as f:
		f.write(str(link))
