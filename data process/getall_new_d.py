...
def travel_get(url, depth, parent):
	...
	if len(ispagination) > 0:	#ispagination
		with open("pagination_d.txt", "a") as f:
			f.white(str(url)+"\n")
...
if '__main__' == __name__:
	...
	# class depth=1
	sub = ET.SubElement(root, 'Classes', {"sunid":"53931","href":"http://scop.berkeley.edu/sunid=53931", "name":"Alpha and beta proteins (a+b)"})

	#startTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')

	startTime = time.time()

	travel_get("http://scop.berkeley.edu/sunid=53931", 1, sub)
	
	# travel_get("http://scop.berkeley.edu/sunid=88552", 6, sub)

	tree = ET.ElementTree(root)
	tree.write('d.xml', encoding="us-ascii", xml_declaration=True, default_namespace=None, method="xml", short_empty_elements=False)
