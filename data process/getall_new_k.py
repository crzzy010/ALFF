...
def travel_get(url, depth, parent):
	...
	if len(ispagination) > 0:	#ispagination
		with open("pagination_k.txt", "a") as f:
			f.white(str(url)+"\n")
...
if '__main__' == __name__:
	...
	# class depth=1
	sub = ET.SubElement(root, 'Classes', {"sunid":"58788","href":"http://scop.berkeley.edu/sunid=58788", "name":"Designed proteins"})

	#startTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')

	startTime = time.time()

	travel_get("http://scop.berkeley.edu/sunid=58788", 1, sub)
	
	# travel_get("http://scop.berkeley.edu/sunid=88552", 6, sub)

	tree = ET.ElementTree(root)
	tree.write('k.xml', encoding="us-ascii", xml_declaration=True, default_namespace=None, method="xml", short_empty_elements=False)
	
	with open("results_k.txt", 'w') as f:
		strs = "acc_num=" + str(acc_num) + "\n" + "pdb num = " + str(count)
		f.write(strs)
	with open("link_k.txt", 'w') as f:
		f.write(str(link))
