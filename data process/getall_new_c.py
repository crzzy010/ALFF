	...
	if len(ispagination) > 0:	#ispagination
		...
		if len(pagelis) > 3:
			...
			if "aria-label" in nextpage.a.attrs.keys() and nextpage.a["aria-label"] == "Next":
				with open("pagination_c.txt", "a") as f:
...
if '__main__' == __name__:
	...
	# class depth=1
	sub = ET.SubElement(root, 'Classes', {"sunid":"51349","href":"http://scop.berkeley.edu/sunid=51349", "name":"b: All beta proteins"})

	#startTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')

	startTime = time.time()

	travel_get("http://scop.berkeley.edu/sunid=51349", 1, sub)
	
	# travel_get("http://scop.berkeley.edu/sunid=88552", 6, sub)

	tree = ET.ElementTree(root)
	tree.write('c.xml', encoding="us-ascii", xml_declaration=True, default_namespace=None, method="xml", short_empty_elements=False)
