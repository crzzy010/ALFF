...
def travel_get(url, depth, parent):
	...
	if len(ispagination) > 0:	#ispagination
		with open("pagination_j.txt", "a") as f:
			f.white(str(url)+"\n")
...
if '__main__' == __name__:
	...
	# class depth=1
	sub = ET.SubElement(root, 'Classes', {"sunid":"58231","href":"http://scop.berkeley.edu/sunid=58231", "name":"Peptides"})

	#startTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')

	startTime = time.time()

	travel_get("http://scop.berkeley.edu/sunid=58231", 1, sub)
	
	# travel_get("http://scop.berkeley.edu/sunid=88552", 6, sub)

	tree = ET.ElementTree(root)
	tree.write('j.xml', encoding="us-ascii", xml_declaration=True, default_namespace=None, method="xml", short_empty_elements=False)
	
	with open("results_j.txt", 'w') as f:
		strs = "acc_num=" + str(acc_num) + "\n" + "pdb num = " + str(count)
		f.write(strs)

	with open("link_j.txt", 'w') as f:
		f.write(str(link))
