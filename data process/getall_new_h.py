...
def travel_get(url, depth, parent):
	...
	if len(ispagination) > 0:	#ispagination
		with open("pagination_g.txt", "a") as f:
			f.white(str(url)+"\n")
	...
	if len(ispagination) > 0:	#ispagination
		...
		if len(pagelis) > 3:
			...
			if "aria-label" in nextpage.a.attrs.keys() and nextpage.a["aria-label"] == "Next":
				with open("pagination_h.txt", "a") as f:
...
if '__main__' == __name__:
	...
	# class depth=1
	sub = ET.SubElement(root, 'Classes', {"sunid":"57942","href":"http://scop.berkeley.edu/sunid=57942", "name":"Coiled coil proteins"})

	#startTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')

	startTime = time.time()

	travel_get("http://scop.berkeley.edu/sunid=57942", 1, sub)
	
	# travel_get("http://scop.berkeley.edu/sunid=88552", 6, sub)

	tree = ET.ElementTree(root)
	tree.write('h.xml', encoding="us-ascii", xml_declaration=True, default_namespace=None, method="xml", short_empty_elements=False)
	
	with open("results_h.txt", 'w') as f:
		strs = "acc_num=" + str(acc_num) + "\n" + "pdb num = " + str(count)
		f.write(strs)
	with open("link_h.txt", 'w') as f:
		f.write(str(link))
